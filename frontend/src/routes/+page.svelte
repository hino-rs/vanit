<script lang="ts">
  ...
  function sendMessage() {
    if (!inputText.trim() || !isPaired || !socket) return;
...
<main class="container">
  ...
  <div class="card send-form">
    <input
      type="text"
      placeholder={isPaired ? 'メッセージを入力...' : 'ペアリング完了後にメッセージを送信できます'}
      bind:value={inputText}
      onkeydown={handleKeydown}
      disabled={!isPaired}
    />
    <button class="btn primary" onclick={sendMessage} disabled={!isPaired || !inputText.trim()}>
      送信
    </button>
  </div>
  ...
  <div class="card logs-card">
    <div class="logs-header">
      <h3>通信ログ</h3>
      {#if logs.length > 0}
        <button class="btn text-btn" onclick={clearLogs}>ログ消去</button>
      {/if}
    </div>
      ...
      {#each logs as log (log.id)}
        <div class="log-item {log.type}">
          <div class="log-content">
            {#if log.type === 'sent'}
              <span class="sender-tag">自分:</span>
            {:else if log.type === 'received'}
              <span class="sender-tag">相手:</span>
            {/if}
